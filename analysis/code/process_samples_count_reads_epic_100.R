...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 101, "-", meth$pos + 99)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 100, "-", meth$pos + 100)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 99, "-", meth$pos + 101)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 98, "-", meth$pos + 102)))))
meth[as.integer(names(t)),]$end_cgn_over <- as.integer(t)

saveRDS(meth, paste0("../../../cfepigenetics_data/Moss_cases_100/", basename(h[case])))
