...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 126, "-", meth$pos + 124)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 125, "-", meth$pos + 125)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 124, "-", meth$pos + 126)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 123, "-", meth$pos + 127)))))
meth[as.integer(names(t)),]$end_cgn_over <- as.integer(t)

saveRDS(meth, paste0("../../../cfepigenetics_data/Moss_cases_125/", basename(h[case])))
