...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 76, "-", meth$pos + 74)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 75, "-", meth$pos + 75)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 74, "-", meth$pos + 76)))))
...
t <- table(subjectHits(findOverlaps(temp, GRanges(paste0(meth$chr, ":", meth$pos - 73, "-", meth$pos + 77)))))
meth[as.integer(names(t)),]$end_cgn_over <- as.integer(t)

saveRDS(meth, paste0("../../../cfepigenetics_data/Moss_cases_75/", basename(h[case])))
